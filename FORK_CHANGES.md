...
|---|---|---|---|---|
| 32 | (this commit) | original | MP rendering / crash safety | **Remote-character rendering via bind-to-existing path.** Replaces the synthetic `RootObjectFactory::create` spawn route (which had been broken on 1.0.68 — 11-arg fastcall signature was being called as 2-arg, corrupting R8/R9/stack args, AVing inside the function at `+0x273`; even when called correctly the resulting char wasn't fully wired into Kenshi's faction lists / AI manager and crashed at game-tick step 9 ~10s later). New path: when the server announces a remote entity, walk `AddToUpdateListMain`-captured Characters, filter by **faction** (`kenshi-online.mod` only — never world NPCs), exclude the **local primary character** (binding it would fight your own input), and bind the netId to the closest position-match (or fallback to any unclaimed same-faction char). Position sync teleports the bound char to where the server says. **Factory pointer** captured by **vtable-match scan** of `GameWorld[0..0x2000]` against the known `RootObjectFactory` vtable RVA `0x16993B0` — works regardless of which `GameWorld` field happens to hold it (KenshiLib's documented `+0x4A0` is wrong on 1.0.68; observed offsets vary per run: `+0x628`, `+0xE8`, etc.). **Per-second bound-char validation** reads the vtable on every linked Character; if invalid (Kenshi streamed it out / freed it), the entity returns to ghost state and re-binds when the char streams back in — prevents stale-pointer AVs at sync-time. Per-player spawn cap raised to 256 (effectively unlimited) backed by the validator. Also: **`MyGuiBridge: setVisible crashed` shutdown noise** softened — widgets nulled before `UnloadLayout`, log demoted from `error` to throttled `warn`. Verified: 21 stable bindings at the Hub, ~3 minutes of gameplay, clean shutdown, no crashes; previous attempts crashed at bind 13 from stale-pointer AV (now caught by validator). Manual findings (offset shifts, hook-collision discovery, vtable RVA) recorded in the `RE_Kenshi-1.0.68` companion repo. |
| 31 | `e7bbce5` `156e651` | original | release infra | **`portforward-test` GitHub Actions workflow.** `workflow_dispatch` job that compiles a tiny C ENet probe on a Linux runner and attempts a real handshake against a user-supplied `ip:port`. Holds the connection for 2s after CONNECT_VERIFY so the server logs `HandleConnect`. Definitive external-reachability test — bypasses NAT loopback, Windows weak-host model, and the false-negatives UDP port checkers give for ENet (which only replies to a valid handshake, not random probes). Self-service: anyone can re-run from the Actions tab when their network changes. |
| 30 | `2c7ee1d` | original | docs | **README: UDP-only port forward + UDP-checker false-negative warning.** Spelled out that KenshiMP is ENet-over-UDP (no TCP forwarding needed) and that yougetsignal-style port checkers will report "closed" even when the path works. |
| 29 | `05cd2c9` | original | MP correctness | **Host flow connects immediately; detect save load from connected state.** Unblocks single-PC host-and-play: the host's local client now connects to the local server before any save load completes, and the loading-gap detector accepts `Connecting` as a valid pre-save phase. |
| 28 | `3773b20` | original | crash safety / scope | **v1.0.68 stability pass: exit-safety + quarantine dead time/speed sync.** Removes ~660 lines of `time_discovery` + `hud_time_probe` diagnostic code that hunted for live host-side time/speed pointers on 1.0.68 — every approach was eliminated (offset 0x700 garbage, TIME_UPDATE / GAME_FRAME_UPDATE never fire, broad memory scan inconclusive). `host_game_speed::Tick` is now a one-shot disabled-warning. Scope reset: speed locked at 1× for everyone on this build, server advances its own clock. Net effect: smaller, leaner DLL (~1.47 MB) with no orphan diagnostic modules. |
