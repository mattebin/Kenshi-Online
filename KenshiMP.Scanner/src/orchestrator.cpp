...
void PatternOrchestrator::ResolveEntry(PatternEntry& entry, uintptr_t address,
    ...
    // pattern intentionally targets an instruction inside the function body (its
    // mid-body mov sequence), and char_tracker_hooks installs an inline hook there
    // rather than a function-prologue hook via MinHook — so a non-aligned address is
