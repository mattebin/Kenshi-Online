...
// on the reentrancy counter can both observe depth==1 simultaneously and
// both take the wrapper's normal-path, corrupting the global RSP/return-
// address save slots and silently terminating the host process when the
// second thread to RET writes the wrong address back to its caller's stack.
